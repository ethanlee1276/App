...
| §4 Movement engine (open → close, steam, RLM) | 🟡 | **The store exists as of 2026-08-10.** `ufc_build` now records every live h2h pull into the shared snapshot history (`linemoves.record_fight_snapshots`) — two rows per book per fight, in `record_snapshots`' exact shape, so `analyze`, `closing_lines`, `opening_odds` and `booksharp` read fight history with zero changes. The reason it was never stored was mundane: this build calls `fetch_event_odds` by hand and the recorder lived inside `apply_odds_to_slate`. Cached pulls are NOT re-recorded — appending a cache re-read would manufacture a "move". What remains is accumulation: open→close, steam and RLM verdicts need a few cards' worth of history, and the movement component stays out of the scorecard until then |
...
| §11 Opener logging / "why it won-lost" categories | 🟡 | **The opener half landed 2026-08-10**: with fight snapshots recording, `linemoves.opening_odds` derives the opener the same way the close is derived — from history, median across the earliest snapshot's books, rather than banked as a column (the banked-close column is the one that spent months wrong while the rebuilt number stayed right). First derivable openers arrive with the next live card. **The cause taxonomy landed 2026-08-10** (`engine/whytags.py`): early-finish, judges, weight-cut, short-notice-showed, injury-in-fight, gameplan, plus the shared tags — controlled vocabulary, free-text note beside it, settled fights only, `launch.py --why-bet` |

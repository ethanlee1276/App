...
| §6 Defense vs position (the matchup under every college pick) | ✅ | **2026-09-23, the NFL's matchup work done for college** (Ethan: "go do the same work you just did for nfl but for CFB"). Every college defence was league average "on purpose"; `engine/cfb/defense.py` now rates what each gives up per game to QBs, RBs, WRs and TEs from our own college logs (FBS against FBS, shrunk 12 games toward last season, nothing before the first game) and `engine/defensevs.effect(sport="cfb")` applies college's own measured strengths (`TRANSFER_CFB`, `python3 cfbdefensefit.py`, 2022-2025 each held out): QB passing yards +3.89% of squared error removed (b 0.82), RB rushing +2.07% (0.87), receivers, tight ends and backs +0.12% to +0.42%, and RB/WR/TE touchdowns ON TOP of the book's implied total (the team's overall scoring record adds nothing beyond it, `tds.defense_multiplier`). Unlike the NFL, a receiver's own position's number predicts him. Every college prop and touchdown row carries `matchup_card` with the school's name. **The NFL's spread and total rules no longer run on college** (`matchup.evaluate_matchup`): measured on college they made passing, WR and TE numbers worse in nearly every season, and the total rule was a flat ×0.95 on almost every college rushing line (NFL baseline 44.6 against college's ~53). **The combined-factor cap is ×0.70–×1.40 for college** (`projection.CAP_BOUNDS`): at the NFL's ×0.85–×1.18 it cut a quarter of college passing rows and most of the measured gain. **Open:** college production runs 7-9% under a player's season-to-date form in every market — a level for the calibration, not a matchup |

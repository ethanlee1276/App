...
| §4 Movement engine | 🟡 | `engine/linemoves.py` open→current, steam, with/against verdict; movement adjusts the quality grade and steam-against can reject. First-mover attribution is measured (which book left its open first, its lead over the second, and whether it is sharp) and is EVIDENCE ONLY — nothing prices from it until `movecheck.py` says movement predicts anything, because `quality.apply_movement` already vetoes picks on an unmeasured signal. Lineup-release re-check happens structurally (build re-runs on refresh after lineups post). Public bet % 📋 (no source), and reverse line movement with it — RLM is defined against the public bet %, so it is blocked by the same gap |
| §5 Pitcher graded first | 🟡 | xERA drives win prob/totals; K props price off whiff-adjusted projections + opposing team K-rate; pitcher-grade certainty is 15% of every grade. Stuff+/Location+, velocity trend, TTO, pitch-count projection 📋 (need pitch-level feed — the best next build) |
| §5 Openers/bullpen games | 🟡 | Probable-pitcher wire catches scratches on refresh; explicit opener flag 📋 |
...
| §6 Pitch arsenal matchup | 📋 | Needs pitch-mix + per-pitch-type hitter data; the platoon engine below is the current best proxy |
...
| §7 Park engine | ✅ | `engine/mlb/parks.py` — per-outcome factors (HR/run/K) per park, roof state live; handedness splits 📋 |
...
| §10 Unified 0–100 grade | ✅ | `engine/mlb/quality.py` with baseball's exact weights (40/15/15/10/10/10 — pitcher & lineup certainty replace NFL's stability & script); A+/A/B+/Pass, **no Leans** |
...
| §11 Why won/lost | 🟡 | Process column (good bet lost / lucky win) via CLV; freeform cause tagging 📋 |

...
| §7 Injury ripple | 🟡 | `engine/injuries.py` holds clouded players + boosts beneficiaries; the QB-dependency watch (`engine/sources/depthcharts.py` `qb_dependency`) stamps a warning on every pass-catcher prop when the QB1 changed week-over-week or carries a concern status — rides the automated refresh via `--depth`; full redistribution model 📋 |

...
def _finish_bet(d: dict, g, config: RuleConfig) -> dict:
    ...
    # …AND WHOSE PRICE IT IS. Moved to `gamebets.attach_books` on
    # 2026-09-09 so baseball gets the same names off the same rule: this
    # block was football-only, and MLB's own `_finish_bet` had no
    # equivalent, so every baseball game card published a price with no
    # shop beside it.
